...
describe("sandbox", () => {
  it("should work when flat is true", async () => {
    ...
    const newSandbox = resultTree.readContent(
      "projects/foo/src/app/feature1/feature1.component.sandbox.ts"
    );
    expect(newSandbox).toMatch(
  ...
  it("should work when flat is false", async () => {
    ...
    tree.create(
      "src/app/feature1/sub-feature1/sub-feature1.component.ts",
      createTestComponent({
        prefix,
        name: "SubFeature1",
      })
    ...
    const newSandbox = resultTree.readContent(
      "src/app/feature1/sub-feature1/sub-feature1/sub-feature1.component.sandbox.ts"
    );
    expect(newSandbox).toMatch(
      /import { SubFeature1Component } from '\.\.\/sub-feature1.component';/
    );
    expect(newSandbox).toMatch(
      /export default sandboxOf\(SubFeature1Component\)/
    );
    expect(newSandbox).toMatch(
      /template: `<app-sub-feature1><\/app-sub-feature1>`/
    );
  });
  it("should work for any prefix", async () => {
    ...
    const newSandbox = resultTree.readContent(
      "src/app/feature1/feature1.component.sandbox.ts"
    );
    expect(newSandbox).toMatch(
      /template: `<fizzbuzz-feature1><\/fizzbuzz-feature1>`/
    );
  });
});
