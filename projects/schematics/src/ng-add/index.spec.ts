...
describe("ng-add", () => {
  it("should work for a default project", async () => {
    ...
    expect(
      angularJson.projects.playground.architect.build.options.outputPath
    ).toBe("dist/playground");
    expect(angularJson.projects.playground.architect.build.options.index).toBe(
      "src/index.html"
    );
    expect(angularJson.projects.playground.architect.build.options.main).toBe(
      ".angular-playground/main.playground.ts"
    );
    expect(
      angularJson.projects.playground.architect.build.options.polyfills
    ).toBe("src/polyfills.ts");
    expect(
      angularJson.projects.playground.architect.build.options.tsConfig
    ).toBe(".angular-playground/tsconfig.playground.json");
    expect(
      angularJson.projects.playground.architect.build.options.assets[0]
    ).toBe("src/favicon.ico");
    expect(
      angularJson.projects.playground.architect.build.options.assets[1]
    ).toBe("src/assets");
    expect(
      angularJson.projects.playground.architect.build.options.styles[0]
    ).toBe("src/styles.css");
    expect(
      angularJson.projects.playground.architect.build.configurations.production
        .fileReplacements[0].replace
    ).toBe("src/environments/environment.ts");
    expect(
      angularJson.projects.playground.architect.build.configurations.production
        .fileReplacements[0].with
  ...
  it("should work for a project created with `ng g app`", async () => {
    ...
    expect(angularJson.projects.playground.sourceRoot).toBe(
      "projects/something/src"
    );
    expect(
      angularJson.projects.playground.architect.build.options.outputPath
    ).toBe("dist/playground");
    expect(angularJson.projects.playground.architect.build.options.index).toBe(
      "projects/something/src/index.html"
    );
    expect(angularJson.projects.playground.architect.build.options.main).toBe(
      ".angular-playground/main.playground.ts"
    );
    expect(
      angularJson.projects.playground.architect.build.options.polyfills
    ).toBe("projects/something/src/polyfills.ts");
    expect(
      angularJson.projects.playground.architect.build.options.tsConfig
    ).toBe(".angular-playground/tsconfig.playground.json");
    expect(
      angularJson.projects.playground.architect.build.options.assets[0]
    ).toBe("projects/something/src/favicon.ico");
    expect(
      angularJson.projects.playground.architect.build.options.assets[1]
    ).toBe("projects/something/src/assets");
    expect(
      angularJson.projects.playground.architect.build.options.styles[0]
    ).toBe("projects/something/src/styles.css");
    expect(
      angularJson.projects.playground.architect.build.configurations.production
        .fileReplacements[0].replace
  ...
  it("should work for a project that contains only a library", async () => {
    ...
    expect(angularJson.projects.playground.sourceRoot).toBe(
      "projects/foo-lib/src"
    );
    expect(
      angularJson.projects.playground.architect.build.options.outputPath
    ).toBe("dist/playground");
    expect(angularJson.projects.playground.architect.build.options.index).toBe(
      "projects/foo-lib/src/index.html"
    );
    expect(angularJson.projects.playground.architect.build.options.main).toBe(
      ".angular-playground/main.playground.ts"
    );
    expect(
      angularJson.projects.playground.architect.build.options.polyfills
    ).toBe("projects/foo-lib/src/polyfills.ts");
    expect(
      angularJson.projects.playground.architect.build.options.tsConfig
    ).toBe(".angular-playground/tsconfig.playground.json");
    expect(
      angularJson.projects.playground.architect.build.options.assets[0]
    ).toBe("projects/foo-lib/src/favicon.ico");
    expect(
      angularJson.projects.playground.architect.build.options.assets[1]
    ).toBe("projects/foo-lib/src/assets");
    expect(
      angularJson.projects.playground.architect.build.options.styles[0]
    ).toBe("projects/foo-lib/src/styles.css");
    expect(
      angularJson.projects.playground.architect.build.configurations.production
        .fileReplacements[0].replace
  ...
  it("should work for a project that contains a library followed by an application", async () => {
    ...
    const angularJson = getJsonFileAsObject(resultTree, "angular.json");
    expect(angularJson.projects.playground.root).toBe(
      "projects/foo-lib-tester"
    );
    expect(angularJson.projects.playground.sourceRoot).toBe(
      "projects/foo-lib-tester/src"
    );
    expect(
      angularJson.projects.playground.architect.build.options.outputPath
    ).toBe("dist/playground");
    expect(angularJson.projects.playground.architect.build.options.index).toBe(
      "projects/foo-lib-tester/src/index.html"
    );
    expect(angularJson.projects.playground.architect.build.options.main).toBe(
      ".angular-playground/main.playground.ts"
    );
    expect(
      angularJson.projects.playground.architect.build.options.polyfills
    ).toBe("projects/foo-lib-tester/src/polyfills.ts");
    expect(
      angularJson.projects.playground.architect.build.options.tsConfig
    ).toBe(".angular-playground/tsconfig.playground.json");
    expect(
      angularJson.projects.playground.architect.build.options.assets[0]
    ).toBe("projects/foo-lib-tester/src/favicon.ico");
    expect(
      angularJson.projects.playground.architect.build.options.assets[1]
    ).toBe("projects/foo-lib-tester/src/assets");
    expect(
      angularJson.projects.playground.architect.build.options.styles[0]
    ).toBe("projects/foo-lib-tester/src/styles.scss");
    expect(
      angularJson.projects.playground.architect.build.configurations.production
        .fileReplacements[0].replace
  ...
  it("should throw if there are no projects", async () => {
    ...
    const errorMessage =
      "Your app must have at least 1 project to use Playground.";
    const promise = runner.runSchematic("ng-add", {}, tree);
    await expect(promise).rejects.toThrowError(errorMessage);
  });
});

const getJsonFileAsObject = (tree: UnitTestTree, filepath: string) =>
  JSON.parse(tree.readContent(filepath));

const createAngularJson = (config: {
  root: string;
  sourceRoot: string;
...
const verifyBasicFiles = (
  tree: UnitTestTree,
  options: { sourceRootPath: string; mainFilePath: string }
) => {
  // package.json
  ...
  const angularPlaygroundJson = getJsonFileAsObject(
    tree,
